...
const PerfumePage = ({ perfume, onClose, onSelectPerfume }: PerfumePageProps) => {
  ...
  const recommendations = PERFUMES.filter((p) => {
    ...
    // Critères : Même marque OU au moins une note de tête commune
    const sameBrand = p.brand === perfume.brand;
    const commonNotes = p.topNotes.some(note => perfume.topNotes.includes(note));
    return sameBrand || commonNotes;
  }).slice(0, 8); // On en prend 8 pour alimenter le carrousel
  ...
  return (
    <div className="relative min-h-screen bg-[#050505] text-white pb-40 overflow-x-hidden selection:bg-amber-200 selection:text-black font-sans">
      ...
      <div className="sticky top-0 z-[100] w-full h-20 bg-black/5 backdrop-blur-[50px] flex items-center justify-between px-10 border-b border-white/[0.02]">
        <span className="text-[8px] uppercase tracking-[0.6em] text-amber-500/60 font-black italic">Haute Parfumerie</span>
        <button onClick={onClose} className="group w-12 h-12 flex items-center justify-center rounded-full border border-white/5 hover:bg-white/10 transition-all">
          <X size={20} className="group-hover:rotate-90 transition-transform duration-700 text-zinc-400 group-hover:text-white" />
        </button>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-8">
          ...
          <div className="lg:col-span-5 space-y-16">
            <motion.div className="group relative aspect-[3/4.2] rounded-[3.5rem] overflow-hidden border border-white/[0.05] shadow-2xl">
              ...
              <div className="absolute bottom-10 left-10 flex items-center gap-4 bg-black/40 backdrop-blur-3xl px-6 py-3 rounded-full border border-white/10">
                ...
                <span className="text-[9px] uppercase tracking-[0.3em] font-medium text-zinc-300 italic">Édition {perfume.year}</span>
              </div>
            </motion.div>
            <div className="px-10 border-l border-white/5">
              <p className="text-zinc-400 text-3xl leading-relaxed font-extralight italic italic">"{perfume.description}"</p>
            </div>
          </div>

          {/* DROITE : JAUGES LIQUIDE GOLD NÉON */}
          <div className="lg:col-span-7 space-y-24">
            <div className="space-y-16">
              <h3 className="text-[12px] uppercase tracking-[0.6em] text-zinc-600 font-bold border-b border-white/5 pb-8 italic italic">Architecture Moléculaire</h3>
              ...
              {stats.map((s, i) => (
                <div key={i} className="group">
                  <div className="flex justify-between items-end mb-6">
                    ...
                    <span className="text-amber-400 text-[24px] font-extralight tracking-tighter italic" style={{ textShadow: '0 0 20px rgba(251, 191, 36, 0.5)' }}>
                       {s.val}<span className="text-[10px] ml-2 opacity-40 font-bold">% VOL</span>
                    </span>
                  </div>
        ...
        {/* --- SECTION CARROUSSEL DE RECOMMANDATIONS --- */}
        <div className="mt-40 pt-20 border-t border-white/[0.04]">
          <div className="flex items-center justify-between mb-16">
            <div className="space-y-2">
              <h3 className="text-[11px] uppercase tracking-[0.6em] text-zinc-600 font-black italic">Collection de Proximité</h3>
              <p className="text-[10px] uppercase tracking-[0.2em] text-amber-500/40">Inspirations olfactives similaires</p>
            </div>
            <div className="flex gap-4">
               <span className="text-[10px] uppercase tracking-widest text-zinc-500 italic">Slide to explore</span>
               <ChevronRight size={14} className="text-zinc-700" />
            </div>
          </div>

          {/* Carrousel avec Framer Motion Drag */}
          <motion.div 
            ref={carouselRef}
            ...
            >
              {recommendations.map((rec) => (
                <motion.button 
                  ...
                  whileHover={{ y: -10 }}
                >
                  <div className="aspect-[3/4.5] rounded-[2.5rem] overflow-hidden border border-white/[0.05] group-hover:border-amber-500/40 transition-all duration-1000 relative bg-zinc-900">
                  ...
                  <div className="space-y-2 px-4">
                    ...
                    <p className="text-[12px] font-extralight uppercase tracking-[0.3em] text-zinc-300 group-hover:text-amber-200 transition-colors truncate italic leading-none">{rec.name}</p>
                  </div>
                </motion.button>
              ))}
            </motion.div>
