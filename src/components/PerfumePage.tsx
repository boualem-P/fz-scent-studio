  ...
  return (
    <div className="relative min-h-screen bg-[#050505] text-white overflow-x-hidden selection:bg-amber-200 selection:text-black font-sans">
      ...
      <div className="sticky top-0 z-[999] w-full pointer-events-none">
        ...
        <header className="relative z-20 h-14 w-full flex items-center justify-between px-5 pointer-events-auto">
          <div className="flex flex-col">
            ...
            {isScrolled && (
              <motion.span initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="text-[10px] uppercase tracking-widest text-zinc-200 font-light">
                {perfume.name}
              </motion.span>
            )}
          </div>
      ...
      {/* ═══════════════════════════════════════
          ...
          PC      (≥1024px) — layout existant
          ═══════════════════════════════════════ */}
      <div className="md:hidden lg:block max-w-5xl mx-auto px-4 pt-4 pb-16 relative z-10">
        ...
        >
          <h1 className="text-3xl lg:text-5xl font-extralight italic tracking-tight uppercase leading-none mb-2">
            {perfume.name}
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          ...
          <div className="space-y-5">
            <h3 className="text-[10px] uppercase tracking-[0.5em] text-zinc-600 font-bold border-b border-white/5 pb-3">Architecture des Essences</h3>
            {stats.map((s, i) => (
              <div key={i} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-amber-400 text-sm font-light italic">
                    {s.val}<span className="text-[8px] ml-1 opacity-40">%</span>
                  </span>
                </div>
                ...
                <div className="flex flex-wrap gap-3 pt-1">
                  {s.notes.map((note, idx) => (
                    <div key={idx} className="flex flex-col items-center gap-1 cursor-pointer group/note"
                      ...
                      <span className="text-[8px] uppercase tracking-wide text-zinc-600 group-hover/note:text-amber-300 transition-colors max-w-[44px] text-center leading-tight">{note.name}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="mt-10 pt-6 border-t border-white/[0.05]">
          <div className="flex items-center justify-between mb-4 px-1">
            <h3 className="text-[10px] uppercase tracking-[0.5em] text-zinc-600 font-black italic">Sillages Analogues</h3>
          ...
          <div ref={carouselRef} className="cursor-grab active:cursor-grabbing overflow-hidden">
            <motion.div drag="x" dragConstraints={{ right: 0, left: -((recommendations.length * 200) - 600) }}
              className="flex gap-4 w-max pb-4 px-1">
              {recommendations.map((rec) => (
                <motion.button key={rec.id} onClick={() => onSelectPerfume(rec)}
                  className="group w-[160px] text-left space-y-2" whileHover={{ y: -4 }}>
                  <div className="perfume-img-container gold-frame !h-[200px] bg-zinc-900/30">
                    <img src={rec.image} className="perfume-img opacity-70 group-hover:opacity-100 transition-opacity" alt={rec.name} />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-end p-3">
                      <div className="flex items-center gap-1">
                        <Plus size={10} className="text-amber-500" />
                        <span className="text-[8px] uppercase tracking-widest text-amber-500">Voir</span>
                      </div>
                    </div>
                  </div>
                  <div className="px-1 space-y-0.5">
                    <p className="text-[8px] uppercase tracking-[0.4em] text-zinc-600 italic">{rec.brand}</p>
                    <p className="text-[11px] font-light uppercase tracking-wide text-zinc-300 group-hover:text-amber-200 transition-colors truncate">{rec.name}</p>
                  </div>
                </motion.button>
              ))}
            </motion.div>
      ...
      {/* ═══════════════════════════════════════
          TABLETTE (768px–1023px) — style Fragrantica
          Image à gauche + infos structurées à droite
          ═══════════════════════════════════════ */}
      <div className="hidden md:block lg:hidden relative z-10">
        <div className="px-6 pt-4 pb-10">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.7 }} className="mb-5">
            ...
            <div className="flex items-center gap-3 mt-1">
              ...
              <div className="flex items-center gap-1.5 text-zinc-500">
                ...
                <span className="text-[9px] uppercase tracking-widest">{perfume.year}</span>
              </div>
            </div>
          </motion.div>

          {/* Ligne principale : image + colonne infos */}
          <div className="flex gap-6 items-start">
            ...
            <div className="flex-shrink-0 w-[240px]">
              ...
              <div className="mt-3 px-3 py-2.5 border-l-2 border-amber-500/30 bg-white/[0.02] rounded-r-lg">
                <p className="text-zinc-500 text-[11px] leading-relaxed font-extralight italic">{perfume.description}</p>
              </div>
            </div>

            {/* Colonne droite — jauges + notes style Fragrantica */}
            <div className="flex-1 space-y-4">
              ...
              {stats.map((s, i) => (
                <div key={i} className="space-y-1.5">
                  {/* Label + % */}
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] uppercase tracking-[0.25em] text-zinc-300 font-medium flex items-center gap-1.5">
                      <span className="text-amber-500/60">{s.icon}</span> {s.label}
                    </span>
          ...
          <div className="mt-8 pt-5 border-t border-white/[0.05]">
            ...
            <div className="cursor-grab active:cursor-grabbing overflow-hidden">
              <motion.div drag="x" dragConstraints={{ right: 0, left: -((recommendations.length * 180) - 700) }}
                className="flex gap-3 w-max pb-3">
                {recommendations.map((rec) => (
                  <motion.button key={rec.id} onClick={() => onSelectPerfume(rec)}
                    className="group w-[140px] text-left space-y-1.5" whileHover={{ y: -3 }}>
                    <div className="perfume-img-container gold-frame !h-[170px] bg-zinc-900/30">
                      <img src={rec.image} className="perfume-img opacity-70 group-hover:opacity-100 transition-opacity" alt={rec.name} />
