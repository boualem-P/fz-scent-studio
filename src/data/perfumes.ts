...
export function matchPerfumes(
  ...
  const scored = candidates.map((perfume) => {
    ...
    score += (totalScore > 0 ? matchScore / totalScore : 0) * 0.55;
    ...
    score += Math.min(contextMatch / 5, 1) * 0.2;
    ...
    if (radarIntensities) {
      ...
      score += Math.max(0, 1 - Math.abs(avgRadar - avgWeight / 2)) * 0.15;
    }

    // D. SIGNATURE (10%)
    const richness = Math.min(perfumeAccords.length / 10, 1);
    const uniqueness = new Set(perfumeAccords).size / Math.max(perfumeAccords.length, 1);
    score += (richness * 0.6 + uniqueness * 0.4) * 0.1;

    return { perfume, matchPercent: Math.round(score * 100) };
  });
  ...
  for (const item of sorted) {
    const accords = getPerfumeAccords(item.perfume.id, [
